...
def guarded_generate(
    ...
    if classifier is not None and final_classifier_check:
        decision = classifier.classify(prompt, final)
        print(
            "\n[classifier:final] "
            f"verdict={decision.verdict} score={decision.score:.3f} "
            f"allow_threshold={decision.allow_threshold:.3f} "
        ...
        if decision.verdict == "block":
            ...
            return {
                ...
                "classifier_verdict": decision.verdict,
                "response": refusal_message,
                "held_response_chars": len(final),
                "final_classifier_check": True,
            }

        if decision.verdict == "uncertain":
            print("[guard] UNCERTAIN: final response held by classifier.")
            return {
                "status": "uncertain",
                "classifier_score": decision.score,
                "classifier_verdict": decision.verdict,
    ...
    return {
        "status": "completed",
        "response": final,
        "final_classifier_check": bool(classifier is not None and final_classifier_check),
    }
