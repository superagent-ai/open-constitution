...
def test_exchange_classifier_scores_unsafe_compliance_for_three_class_model():
    classifier = ExchangeClassifier(
        model=FakeModel([2.0, 2.0, 6.0]),
        tokenizer=FakeTokenizer(),
        device="cpu",
        max_length=16,
        allow_threshold=0.3,
        block_threshold=0.7,
        label_mapping={"0": "safe_compliance", "1": "safe_refusal", "2": "unsafe_compliance"},
    )

    decision = classifier.classify("prompt", "partial")

    assert decision.verdict == "block"
    assert decision.score == pytest.approx(decision.label_scores["unsafe_compliance"])
    assert decision.label_scores["safe_refusal"] < decision.score
